...
local function self_aff(name)
	local a = Legacy and Legacy.Curing and Legacy.Curing.Affs and Legacy.Curing.Affs[name]
...
local function is_parry_bypassed(limb)
	return has_aff("haemophilia", CONFIG.PARRY_AFF_THRESHOLD) or targetparry ~= (limb:gsub(" ", "")) or is_prone()
end
...
local function current_morph()
	...
	return name and Legacy and Legacy.Tannivh and Legacy.Tannivh.morph or nil
end
...
function sentinel.on_balance(interval)
	...
	sentinel.state.next_bal_timer = tempTimer(math.max(0, interval - prearm), function()
...
function sentinel.status()
	...
	cecho(
		"\n<yellow>|   <white>SKULLBASH: " .. ((is_prone() and is_limb_broken("head")) and "<green>READY" or "<red>no")
	)
	cecho(
		"    <white>WRENCH: "
